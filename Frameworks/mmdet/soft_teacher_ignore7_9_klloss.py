...
class SoftTeacher(MultiSteamDetector):
    ...
    def unsup_rcnn_cls_loss(
        ...
        for ind,i in enumerate(sampling_results):
            for jind,j in enumerate(i.pos_assigned_gt_inds):
                if gt_bboxes2[ind][j][4]<0.9:
                    bbox_targets[1][ind*512+jind]=0
                    ignore_num+=1
                    student_soft=torch.cat([student_soft,bbox_results['cls_score'][ind*512+jind].unsqueeze(0)],dim=0)    # get the ind th photo's jind th sample
      ...
      #  print(teacher_soft)
        ...
        loss["loss_cls"] = loss["loss_cls"].sum() / max(bbox_targets[1].sum()+ignore_num, 1.0)
       ...
       # if self.sign%200==0:
           ...
           # print(ignore_num)
      #  print(ignore_num)
        loss_soft = loss_kl.mean(1).sum()/max(bbox_targets[1].sum()+ignore_num, 1.0)
        self.sign+=1
    ...
    def unsup_rcnn_reg_loss(
        ...
        **kwargs,
        ...
        gt_bboxes, gt_labels, _ = multi_apply(
            ...
            [bbox[:, :4] for bbox in pseudo_bboxes],
            pseudo_labels,
            [-bbox[:, -4:].mean(dim=-1) for bbox in pseudo_bboxes],  # change 
